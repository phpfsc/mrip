<?php
	...
	if ($qry)
	{
		...
		else
		{
			if ($Type=="Plain")
			{
				$result=mysqli_query($con2,"select fRate from shaft_pinion_rate_master where fDMValue =$DMValue and cType ='$DMType' and iTeeth =$Teeth and fFaceFrom <= $Face  and fFaceTo >=$Face and cFaceType ='$FaceType'");
			}
			else
			{
				$result=mysqli_query($con2,"select fRate from shaft_pinion_rate_master where fDMValue =$DMValue and cType ='$DMType' and fFaceFrom <= $Face  and fFaceTo >=$Face and cFaceType ='$FaceType'");
			}
